...
def validate_config(config_obj: Config) -> bool:
    ...
    if 'risk_weights' in config_obj.FRAMEWORK_CONFIG:
        weights = config_obj.FRAMEWORK_CONFIG['risk_weights']
        total = sum(weights.values())
        if abs(total - 1.0) > 0.001:
            raise ConfigurationError(f"Risk weights must sum to 1.0, got {total}")

    # Validate thresholds are in order
    if 'confidence_thresholds' in config_obj.FRAMEWORK_CONFIG:
        thresholds = config_obj.FRAMEWORK_CONFIG['confidence_thresholds']
        if not (thresholds['low'] < thresholds['medium'] < thresholds['high']):
            raise ConfigurationError("Confidence thresholds must be in ascending order")
